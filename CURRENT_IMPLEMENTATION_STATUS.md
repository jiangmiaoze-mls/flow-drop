...
完整的当前选择、解析、协商、上传、校验、完成与恢复链路见 [FILE_TRANSFER_PIPELINE.md](FILE_TRANSFER_PIPELINE.md)。现有 JS 发送器已在真机反馈中暴露 UI 阻塞与状态滞后，不能视为可验收实现；拟替换架构、V3 协议、原生执行器、Agent 写入队列和迁移策略见 [FILE_TRANSFER_REDESIGN.md](FILE_TRANSFER_REDESIGN.md)。该文档已补充单向 Zustand/SQLite 投影、旧任务不再执行 JS 上传器、完成屏障、Application 级生命周期、HMAC 测试向量、Agent SQLite 基准测试，以及基于持久化分块摘要的可恢复 `contentRoot` 和 Agent worker 最终落盘校验。它仍是技术方案，不表示 V3 已实现。
...
- 文字会创建单项任务、以 UTF-8 计算 SHA-256、创建 Agent 任务并请求完成；文件在选择后会立即以 `preparing`（界面显示“解析中”）状态写入 SQLite 队列。`DocumentPicker` 已先将选中文件复制进应用缓存，准备流程只将其移动到应用文档目录的 `flowdrop-outgoing`，不再预计算完整 SHA-256。文件移动完成即变为 `queued`（“待传输”）。SQLite 将准备进度 `prepared_bytes` 与已确认上传进度 `transferred_bytes` 分列保存；新建文件任务使用协议 `v2`，上传器按文件顺序读取分块时增量计算完整 SHA-256，完成请求再携带摘要，Agent 重新计算 `.part` 后比对并原子落盘。正常新传输只读取发送文件一遍；续传的已确认块仍会被本地读取以重建摘要，但不会重复上传。Agent 保留 `v1` 接收兼容，旧 Agent 不支持 `v2` 发送端，必须更新并重启。应用重新进入传输页会查询未终态远端任务：远端已完成则补写本地完成状态，远端仍进行中则本地暂停等待用户继续。可恢复网络错误会以 5、10、20 秒退避重新协商，最多三次；尚未实机验证。
- 发送器会在 Agent 接收任务创建响应前显示 `negotiating`（“正在协商”），创建响应返回后立即切换 `transferring`，不再等待首块确认才更新 UI。创建成功和每个分块投递后均主动让出一帧，因为 React Native 的文件读取、分块摘要和 HMAC 是同步 JS 工作；否则多个连续分块可让 Web 已开始接收而 App 尚未绘制新状态，且暂停点击无法即时响应。传输没有按秒限速器；新任务采用 1 MiB 分块、最多 4 个在途请求的滑动窗口，以控制 React Native 中二进制请求体、HMAC 与 `fetch` 副本造成的峰值内存。已经创建的 4 MiB 任务保持原断点偏移，但恢复时退化为单请求。进度使用 Agent 分块响应返回的累计已确认字节，而非发送端本地递增值；显示会按 1 MiB 确认粒度跳变。传输中额外显示基于连续 Agent 确认字节计算的瞬时平滑速率，不写入历史记录。每个 Agent 请求设有 15 秒超时；可恢复失败会以 5、10、20 秒退避重新协商，最多三次。尚未实机验证。
- 多选文件时，移动端会先按选择顺序创建全部 SQLite 任务并显示为“解析中”，完成后台解析后变为“待传输”，然后 FIFO 逐项发送；队首成功后自动发送下一项。队首失败、暂停或等待对端时，后续任务保持待传输，不会绕过队首并发发送。
- 发送页不显示已完成或已取消任务，但两者都保留在 SQLite 历史中。失败或“等待对端”任务提供“重发”；发送中任务同时提供暂停和取消，暂停后提供继续和取消，按钮只保留图标及无障碍标签。暂停、继续和取消先乐观写入移动端 SQLite 并立即刷新 UI，再调用 Agent 已认证的 `/pause`、`/resume`、`/cancel` 接口；Agent 会持久化对应状态并发布 `transfer.changed` 供本机 Web 刷新。远端操作失败时，移动端会回滚到可继续的本地状态并显示错误；仅 Agent 明确返回 `TRANSFER_NOT_FOUND`（接收端从未创建该任务）时保留本地乐观状态。暂停不会删除 Agent 暂存块；取消会清理 Agent 暂存块。应用重启后会查询 `queued`、`transferring`、`verifying` 与等待对端等本地非终态任务；若 Agent 已有进行中任务，移动端请求 Agent 同步暂停并使用其实际已确认字节，避免手机已退出而 Agent 仍显示传输中。Agent 将任务分块大小和已确认块索引持久化并返回，因此恢复时可补发缺失块。尚未实机验证暂停、取消、恢复和网络竞态行为。
